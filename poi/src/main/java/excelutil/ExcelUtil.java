...
public class ExcelUtil {
     ...
     * @return
     */
    private static String getCellTypeByEnum(CellType cellType) {
        switch (cellType) {
            case BLANK:
                return "Null type";
            case BOOLEAN:
...
//            LG.error(e.toString(), e);
    ...
    private static String validateCell(Cell cell, Field field, int cellNum) {
        ...
        if (cell == null
            ...
            List<CellType> _cellTypes = Arrays.asList(cellTypes);

            // 如果類型不在指定範圍內,並且沒有默認值
            if (!(_cellTypes.contains(cell.getCellType()))
                    || StringUtils.isNotBlank(annoCell.defaultValue())
                    && cell.getCellType() == CellType.STRING) {
                StringBuilder strType = new StringBuilder();
                for (int i = 0; i < _cellTypes.size(); i++) {
                    CellType cellType = _cellTypes.get(i);
                    strType.append(getCellTypeByEnum(cellType));
                    if (i != _cellTypes.size() - 1) {
                        strType.append(",");
                    }
                }
                result =
